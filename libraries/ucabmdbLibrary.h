...
void driveSeasons(Serie **serie, Serie *serie_head){
    ...
    switch(season_option){
      ...
      case 4:
      {
        ...
        if (findSeasonByName(aux->season_head, season_name) == true){
          ...
          printChaptersList(aux->chapters_head);
          driveChapters(&aux->chapters_head, serie_head);
...
void showSerieByName(Serie **serie_head){

  if (serie_head){

    Serie *aux = *serie_head;

    string serie_name = searchSerie();

    bool bol = findSerieByName(aux, serie_name);
